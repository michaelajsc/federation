...
export class JoinSpecDefinition extends FeatureDefinition {
  ...
  addElementsToSchema(schema: Schema): GraphQLError[] {
    ...
    // Note that this graphs argument is plural, since the same directive
    ...
    // The @join__type and @join__field directives represent applied directives
    // from subgraphs using their `directives: [AppliedDirective!]` argument,
    // which covers types and fields. For everything else (e.g. schema
    // elements), there's @join__directives, which encodes the same information
    // with its `applied: [AppliedDirective!]` argument.
    // joinDirective.addArgument('applied', new ListType(new NonNullType(AppliedDirective)));
    // joinType.addArgument('directives', new ListType(new NonNullType(AppliedDirective)));
    // joinField.addArgument('directives', new ListType(new NonNullType(AppliedDirective)));
